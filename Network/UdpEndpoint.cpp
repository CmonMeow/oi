...
unsigned __int32 crc32(unsigned __int32 crc, const unsigned char* buf, __int64 len)
{
	...
	while (len >= 8)
	{
		crc = crc_table[((__int32)crc ^ (*buf++)) & 0xff] ^ (crc >> 8);
		crc = crc_table[((__int32)crc ^ (*buf++)) & 0xff] ^ (crc >> 8);
		crc = crc_table[((__int32)crc ^ (*buf++)) & 0xff] ^ (crc >> 8);
		crc = crc_table[((__int32)crc ^ (*buf++)) & 0xff] ^ (crc >> 8);
		crc = crc_table[((__int32)crc ^ (*buf++)) & 0xff] ^ (crc >> 8);
	...
	{ 
		...
		for (batchIt = 0; batchIt++ < PacketsPerSendBatch;)
		{
			...
			{ 
				...
				__int32 serror = 0;
				__int32 errLen = sizeof(serror);
				getsockopt(peer->sock, SOL_SOCKET, SO_ERROR, (char*)&serror, &errLen);
				WSASetLastError(0); 
		...
		for (batchIt = 0; batchIt++ < PacketsPerSendBatch;)
		{
			...
			if (!channel || !channel->prepareNextPacket())
			{
				...
				if (!peer->chMap.empty())
				{
					auto it = peer->chMap.begin();
					if (hasOrigin)
					{
						auto originIt = peer->chMap.find(originKey);
						if (originIt != peer->chMap.end())
						{
							it = originIt;
							++it;
							if (it == peer->chMap.end())
								it = peer->chMap.begin();
						}
					}

					for (size_t checked = 0; checked < peer->chMap.size(); ++checked)
					{
						if (it == peer->chMap.end())
							it = peer->chMap.begin();
						channel = it->second;
