...
public:
    ...
    void toggle(HostDirectoryClient& directory){visible=!visible;if(visible)directory.refresh();}
    void show(HostDirectoryClient& directory){visible=true;directory.refresh();}
    ...
    void update(cNetworkRuntime& network,HostDirectoryClient& directory) {
        bool connected=network.hasConnection();if(wasConnected&&!connected)show(directory);wasConnected=connected;
        if(!shown(network)||!input.leftClick())return;
        const int top=App.size.y-34;
        for(size_t i=0;i<directory.entries.size();++i)if(hit(20,top-64-(int)i*18,App.size.x-40,18)){
            std::string address=HostDirectory::addressText(directory.entries[i]);input.KeyUp(VK_LBUTTON);
            if(network.connectTo(address))visible=false;return;
        }
        if(hit(20,48,128,22))directory.refresh(directory.page);
        else if(hit(156,48,128,22)&&directory.page>0)directory.refresh(directory.page-1);
        else if(hit(292,48,128,22)&&directory.page+1<directory.pages)directory.refresh(directory.page+1);
        else if(hit(App.size.x-140,48,128,22))visible=false;
        if(hit(12,44,App.size.x-24,top-44))input.KeyUp(VK_LBUTTON);
    }
    void draw(const cNetworkRuntime& network,const HostDirectoryClient& directory) const {
        if(!shown(network))return;
        const int top=App.size.y-34;
